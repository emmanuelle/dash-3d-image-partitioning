...
// multiple figures each showing views of the same slice of many possible
// slices. For example, if we have 10 slices, and 2 figures, then the returned data
// structure will be Array[10][2], where each entry is a variable-length array
// depending on the number of shapes in that slice+figure combination.
...
undo_data,
// an array of graph.figure objects (usually from a bunch of State variables
// wrapping graph.figure components)
graph_figures,
...
slice_index,
    ...
    console.log("graph_figures"); console.log(graph_figures);
    ...
    // figures, we just use the shapes already in the figure if we didn't get any new figures passed in through the inputs to the callback
    let shapes = graph_figures.map(
         f=>((f.layout.shapes == undefined) ? [] : json_copy(f.layout.shapes)));
            ...
            zip(graph_relayoutData_ids,graph_relayoutData).foreach(
                function(t,i){
                    if ((triggered == t[0]) && ("shapes" in t[1])) {
                        shapes[slice_index][i] = t[1].shapes;
                    }
                }
            );
